...
$search_query = "SELECT schedule.s_id,schedule.departure_time,schedule.arrival_time,schedule.price,route_find.route_id,bus.type,bus.no_of_seat,bus_operator.bus_operator_name,bus_operator.email,bus_operator.phone_no,route.title,route_find.c_name AS route_city_detail
FROM (
    SELECT route_detail.route_id, 
    GROUP_CONCAT(city.c_name ORDER BY route_detail.location ASC SEPARATOR '-') AS c_name, 
    GROUP_CONCAT(city.c_id ORDER BY route_detail.location ASC SEPARATOR '-') AS c_id, 
    GROUP_CONCAT(route_detail.location ORDER BY route_detail.location ASC SEPARATOR '-') AS location 
    FROM route_detail 
    INNER JOIN (
        SELECT route_detail.route_id,route_detail.location,route_detail.city_id 
        FROM route_detail 
        INNER JOIN (
            SELECT route_detail.route_id,route_detail.location,route_detail.city_id AS source_id 
            FROM route_detail WHERE route_detail.city_id = '$source_id' 
        ) AS A 
        ON A.route_id = route_detail.route_id AND A.location < route_detail.location 
        AND route_detail.city_id = '$destination_id'
    ) AS route_detail_new 
    ON route_detail.route_id = route_detail_new.route_id 
    INNER JOIN city ON city.c_id = route_detail.city_id 
    GROUP BY route_detail.route_id
) as route_find	
INNER JOIN route ON route.r_id = route_find.route_id
